...
from src.llms.text import BaseTextModel
from src.retrieval.common import SummaryRetriever, MarkdownGenerator
from src.retrieval.models import DocumentFilter, DocumentFilterWithDetails, DocumentSummary, DocumentMetadata
...
class MetadataSelector:
    ...
    def run_with_details(
        ...
        for file_id, metadata in metadata_dict.items():
            ...
            # Find original summary (summaries are DocumentSummary objects)
            summary_obj = next((s for s in summaries if s.file_id == file_id), None)
