...
5. **`file_type`** (string)
   ...
   - Usa el valor exacto del campo `file_type` del documento
...
5. **Todos los documentos**: Si la consulta no especifica documentos particulares pero es general, incluye metadata para todos los documentos disponibles (típicamente con valores `null` excepto `file_type`).
