...
class SummaryRetriever:
    ...
    def get_all_summaries(self) -> List[DocumentSummary]:
        ...
        try:
            # Fields we want to retrieve (schema uses file_type, pages, chapters, full_images)
            output_fields = [
                "file_id",
                "file_name",
                "file_type",
            ...
            # Process results (fields match Milvus schema)
            summaries = []
            for result in results:
                summary = {
                    ...
                    "full_images": result.get("full_images", "0"),
