...
class OpenAIEmbedder(BaseEmbedder):
    ...
    def _from_config(cls, config: dict) -> 'OpenAIEmbedder':
        ...
        # Calculate dimensions based on model
        if instance.model in cls.MODEL_DIMENSIONS:
            instance.dimensions = cls.MODEL_DIMENSIONS[instance.model]
        else:
            raise ValueError(
                f"Unknown model '{instance.model}'. "
                f"Supported models: {list(cls.MODEL_DIMENSIONS.keys())}. "
                f"If this is a new model, please add it to MODEL_DIMENSIONS mapping."
            )
        return instance
