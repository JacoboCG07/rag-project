...
def validate_image_structure(image: Any) -> bool:
    ...
    if not isinstance(image['image_number_in_page'], int) or image['image_number_in_page'] < 1:
        return False
...
def validate_images_list(images: List[Any]) -> Tuple[bool, str]:
    ...
    for idx, image in enumerate(images):
        if not validate_image_structure(image):
            return False, f"Image at index {idx} has invalid structure. Expected dict with fields: page, image_number_in_page, image_number, image_base64, image_format"
